...
# ys=[]
threshold=5
path='/home/neuron/桌面/test/2_200_100/8_2.swc'
img=loadswc(path)
...
for point in tswc:
    if point[1]==6:#focus only on the end point
        if ((point[2] not in range(b*cropx-cropx+threshold,b*cropx-threshold))or(point[3] not in range(a*cropy-cropy+threshold,a*cropy-threshold))):#check whether it fits the y field
                print(point)
# x=tswc[:,2:3]
# y=tswc[:,3:4]
